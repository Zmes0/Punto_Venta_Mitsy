...
class HistorialVentasWindow:
    ...
    def borrar_venta(self):
        """Elimina ventas seleccionadas y reorganiza los números de venta."""
        selection = self.detail_tree.selection()
        ...
        if not messagebox.askyesno("Confirmar", 
                                   ...
                                   "Los números de venta se reorganizarán si un número de venta se elimina por completo."):
            return
        ...
        for item in selection:
            ...
            if venta_id:
                venta_ids_a_borrar.append(venta_id)
        
        if venta_ids_a_borrar:
            try:
                db.delete_ventas_and_reorganize(venta_ids_a_borrar)
                messagebox.showinfo("Éxito", "Venta(s) eliminada(s) y reorganizada(s) correctamente.")
            except Exception as e:
                messagebox.showerror("Error", f"Ocurrió un error durante la eliminación: {e}")
                db.conn.rollback()
